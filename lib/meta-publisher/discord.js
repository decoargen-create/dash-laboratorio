...
/**
 * Función base. Recibe el webhookUrl como parámetro (NO lee env).
 * Si webhookUrl es null/undefined/vacío, skipea y loguea.
 *
...
async function postWebhook(webhookUrl, content) {
  if (!webhookUrl) {
    console.warn('[meta-publisher] webhookUrl vacía — skipeando reporte Discord');
 ...
 */
export async function reportEmptyRun(webhookUrl, { reason, hour }) {
  const h = hour || currentRunHourAR();
  const motivo = reason ? ` _(${reason})_` : '';
  return postWebhook(webhookUrl, `✅ **Meta Ads Publisher — ${h} ART**: sin campañas nuevas para publicar en este run${motivo}.`);
}
 ...
 */
export async function reportFatalError(webhookUrl, { message, hour, hint }) {
  const h = hour || currentRunHourAR();
  const tail = hint ? `\n💡 ${hint}` : '';
  return postWebhook(webhookUrl, `🚨 **Meta Ads Publisher — ${h} ART**: error crítico\n\`\`\`\n${String(message).slice(0, 1500)}\n\`\`\`${tail}`);
}
 ...
 * }} payload
 */
export async function reportRun(webhookUrl, { hour, adAccountId, published, skipped }) {
  const h = hour || currentRunHourAR();
  const lines = [];
  ...
  return postWebhook(webhookUrl, lines.join('\n'));
}
