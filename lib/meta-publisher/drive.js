...
/**
 * @param {{ kind: 'service-account' | 'oauth-user', json?: string, accessToken?: string, refreshToken?: string }} driveAuth
 * @returns {Promise<import('googleapis').drive_v3.Drive>}
 */
export async function getDriveClient(driveAuth) {
  ...
  if (driveAuth.kind === 'oauth-user') {
    ...
    // Cuando se setea refresh_token y client_id+client_secret, googleapis
    // refresca el access_token solo si está vencido. En Fase 5 vamos a
    // pasar también clientId/clientSecret para habilitar ese auto-refresh.
